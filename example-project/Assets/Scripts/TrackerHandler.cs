...
public class TrackerHandler : MonoBehaviour
{
    ...
    public KinectCameraController k_mainCamera;
    public RATUser user;
    ...
    public void updateTracker(BackgroundData trackerFrameData)
    {
        ...
        //SkeletonPosition position = new SkeletonPosition(skeleton, new List<JointId>{JointId.Head}, k_mainCamera.transform.position);
        //position = smoother.ReceiveNewSensorData(position, true);
        //renderSkeleton(skeleton, 0);
        //mainCamera.GetHeadPosition(skeleton);
        Vector3 leftEyePos = new Vector3(skeleton.JointPositions3D[(int)JointId.EyeLeft].X, -skeleton.JointPositions3D[(int)JointId.EyeLeft].Y, skeleton.JointPositions3D[(int)JointId.EyeLeft].Z);
        Vector3 rightEyePos = new Vector3(skeleton.JointPositions3D[(int)JointId.EyeRight].X, -skeleton.JointPositions3D[(int)JointId.EyeRight].Y, skeleton.JointPositions3D[(int)JointId.EyeRight].Z);
        ...
        Vector3 headPos = (leftEyePos + rightEyePos) / 2;
        //Quaternion headRot = Y_180_FLIP * new Quaternion(skeleton.JointRotations[(int)JointId.Head].X, skeleton.JointRotations[(int)JointId.Head].Y,
        //        skeleton.JointRotations[(int)JointId.Head].Z, skeleton.JointRotations[(int)JointId.Head].W) * Quaternion.Inverse(basisJointMap[JointId.Head]);
        if (k_mainCamera != null)
        {
            k_mainCamera.UpdatePosition(headPos);
        }
        if (user != null)
        {
            user.SetHeadPosition(headPos);
        }
