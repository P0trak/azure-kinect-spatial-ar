...
{
    [DataContract]
    public class KinectAzureCalibration
    {
        public const int depthImageWidth = 640;
        public const int depthImageHeight = 576;
        ...
        public void RecoverCalibrationFromSensor(Device kinectSensor)
        {
            ...
            for (float x = -2f; x < 2f; x += 0.2f)
                for (float y = -2f; y < 2f; y += 0.2f)
                    for (float z = 0.4f; z < 4.5f; z += 0.4f)
                    {
                        ...
                        Calibration calibration = kinectSensor.GetCalibration();
                        ...
                        Vector3 kinectColorPoint = (Vector3)calibration.TransformTo3D(kinectCameraPoint, CalibrationDeviceType.Unknown, CalibrationDeviceType.Color);
                        ...
                        Vector3 kinectDepthPoint = (Vector3)calibration.TransformTo3D(kinectCameraPoint, CalibrationDeviceType.Unknown, CalibrationDeviceType.Depth);
